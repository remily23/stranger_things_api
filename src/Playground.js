...
for (actors of unfilteredActors){
    if (!filteredActors.includes(actors.author)){
       filteredActors.push(actors.author); 
    }
}

const newList = filteredActors.map((actor) => {
    return actor.toLowerCase().split(/[\s.]+/).join("-")
})
